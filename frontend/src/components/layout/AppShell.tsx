...
function Icon(props: { name: "menu" | "home" | "results" | "graph" | "share" | "ai" | "mail" | "tools" | "collapse" }) {
  ...
  switch (props.name) {
    ...
    case "results":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none">
          ...
          <path d="M5 4h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1Z" stroke="currentColor" strokeWidth="2" />
        </svg>
      );
    case "graph":
      return (
    ...
    case "share":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none">
          ...
          <path d="M7 12a3 3 0 1 0 0 0Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          <path d="M16 20a3 3 0 1 0-2.83 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          <path d="M9.5 11l5-3M9.5 13l5 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
        </svg>
      );
    case "ai":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none">
          <path d="M12 2v3M12 19v3M4.5 4.5l2.1 2.1M17.4 17.4l2.1 2.1M2 12h3M19 12h3M4.5 19.5l2.1-2.1M17.4 6.6l2.1-2.1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    ...
    case "tools":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none">
          <path d="M14 7 7 14m0-4v4h4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          <path d="M10 7h10v10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
        </svg>
      );
    case "collapse":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none">
          <path d="M14 7 9 12l5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
  }
}
...
function Breadcrumbs() {
  ...
  const crumbs = useMemo(() => {
    ...
    const acc: Array<{ label: string; href: string }> = [{ label: "Home", href: "/" }];
    let href = "";
    ...
    <nav className="flex items-center gap-2 text-xs text-slate-500">
      {crumbs.map((c, idx) => (
        <span key={c.href} className="flex items-center gap-2">
          ...
          {idx < crumbs.length - 1 ? <span className="text-slate-300">/</span> : null}
        </span>
      ))}
    </nav>
  );
...
function SidebarNav(props: { collapsed: boolean; onNavigate?: () => void }) {
  ...
  const iconBase =
    "h-8 w-8 rounded-xl border border-sky-100 bg-white/70 shadow-sm backdrop-blur flex items-center justify-center text-sky-700";

  const Item = (p: { to: string; label: string; icon: Parameters<typeof Icon>[0]["name"]; exact?: boolean }) => (
    <NavLink
      to={p.to}
      end={p.exact}
      ...
      className={({ isActive }) =>
        classNames(
          ...
          isActive
            ? "border border-sky-100 bg-white/60 text-sky-800 shadow-sm backdrop-blur"
    ...
    >
      ...
      {props.collapsed ? null : <span className="text-[13px]">{p.label}</span>}
    </NavLink>
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-white text-slate-900">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                <div className="text-sm font-semibold text-slate-900">AI Spend Audit Tool</div>
                <div className="text-xs text-slate-500">Automation-grade spend insights</div>
              </div>
          </div>
      ...
      >
        ...
        >
          ...
          >
            <div className={classNames("flex items-center justify-between gap-2", collapsed ? "mb-2" : "mb-2.5")}>
              {collapsed ? null : <div className="text-xs font-semibold text-slate-500">Navigation</div>}
              <button
                type="button"
                onClick={() => setCollapsed((v) => !v)}
                className="hidden rounded-xl border border-slate-200 bg-white/70 px-2 py-2 text-slate-700 shadow-sm hover:bg-white focus:outline-none focus:ring-4 focus:ring-sky-200 lg:inline-flex"
                aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
              >
                <span className={classNames("transition-transform", collapsed ? "rotate-180" : "")}>
                  <Icon name="collapse" />
                </span>
              </button>
            </div>
